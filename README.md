...
Загрузочный диск должен быть от редхатоподобного дистрибутива 7й версии. Я использовал `CentOS-7-x86_64-Minimal-1810.iso`. Не должно быть ' в публичном ключе (в комментариях). Скрипт делает:
...
- Устанавливает на них Linux из указанного образа: один раздел винчестера, без swap. Переключение раскладок клавиатуры на __Ctrl-Shift__, пароль у root _"changeme"_ (полезен на случай захода через консоль). Хоть и будет писаться, что создаётся еще пользователь vboxuser, он не создаётся. Инсталятор автоматический настраивает сервис точного времени через `chronyd` и публичный пул. Потом следует брать время с надежных серверов - официальных серверов датацентра. При инсталяции записывается публичный ключ root'а для ssh (тот что указан вторым аргументом).
- Запускает, забирает у них публичный ключ ssh, запивает его в файл директорию своего запуска.
- Выключает, делает снэпшот `create_vms`.

Время выполнения 11 минут на MacBook Pro. Операция обратная `destroy_vms`: уничтожает всё, что насоздавал предыдущий скрипт.
...
Время выполнения 22 минуты на MacBook Pro. Обратная операция `rollback2create_vms`.
...
- Создает 4 БД (если точнее кластера PostgreSQL), в нулевом кластере 2 БД, на остальных по одной.
...
- В Pacemakert создает ресурсы в соответствии к созданным DB, прописывает для них плавающие IP.
- Выключает кластера, виртуалки, создает снэпшот `configure`.

Время выполнения 6 минут на MacBook Pro. Обратная операция `rollback2install_soft`.
...
- Ждет появления пинга на плавающий IP мастера (как критерийr, что кластер встал).
- Создает пользователя _heartbeat_ и БД.
- Выключает кластера, виртуалки, делает снэпшот `heartbeat`.

Время выполнения 3 минуты на MacBook Pro. Обратная операция `rollback2configure`.
...
Поскольку все виртуалки объеденены в группы, согласно кластерам, запускать и останавливать в приложении VirtualBox можно через правую кнопку мыши на имени группы(кластера). Но, приложение VirtualBox иногда от этого вылетает, помогает перезапуск. А иногда виснет так, что и при перезапуске продолжает виснуть. :) Помогает `killall VBoxHeadless` (на хосте). Запускать надо либо целиком группу `Tuchanka` (__не рекомендуется в данной версии VirtualBox__), либо по очереди запуская `Witness4Tuchanka` и подгруппу/подгруппы, например `Tuchanka0`. После этого зайди на любую машину нужного кластера и выполнить команду:
...
Внутри виртуалки находящейся в кластере, состояние кластера можно мониторить скриптом `mon`, находится в /root/bin. Востанавливать БД после того как она рассинхронизировалась можно скриптами типа `restore0a`, лежат в /root/bin. Файлы типа `restore0a` или `restore1` только удаляют текущую директорию БД и копируют БД с текущего мастера. А файлы `restore` (созданы только для тестового стенда) выполняют рутинную работу при тестировании: запускают файл типа `restore1`, добавляют ноду в кластер `pcs cluster start` и удаляют сообщения о старых ошибках из кластера. На проде использовать такие скрипты считаю нецелесообразным, т.к. если причина отказа работы сервера неизвeстна, то сисадмин должен контролировать каждый шаг.

Отключение, сначала надо на одной из машин кластера выполнить:

	pcs cluster stop --all

Потом нажав правую кнопку мыши на заголовке нужной группы нажать: `Close/ACPI Shutdown`.

## Heartbeat
Если в конце установки была установлена БД _heartbeat_ скриптом `script/hearbeat`, то можно использовать скрипты из папки `heartbeat/` (на хосте). Все скрипты коннектятся к плавающим IP, на которых оказываются услуги и пишут/читают текущее время в таблицу heartbeat (состоющую из одной строчки, одного столбца) с частой 0.1 секунды (в идеале).

Все скрипты начинающиеся на heart пишут в плавающие IP принадлежащие мастеру соответствующей БД. Скрипты которые начинаются на reader читают из плавающих IP принадлежащих рабам соответствующей БД, за исключением tuchanka0. Т.к. там по схеме рабы услуги не оказывают (схема с уплотнением), поэтому плавающих IP у них нет, поэтому `reader0a` и `reader0b` читают из мастерских IP.
...
Выключение кластера:
- На любой из нод кластера: `pcs cluster stop --all`.
- После чего в управлялке через правую мышу `Close/ACPI Shutdown` на группе Tuchanka1 и Witness4Tuchanka.

Другие тесты будут отличаться способом, каким имитируется неисправность, и спецификой работы каждого кластера. На текущий момент (как уже писал в ToDo) watchdog демон отрабатывает остановку наблюдаемых демонов, но не их зависание и, как следствие, некорректно отрабатываются неисправности приводящие к зависанию corosync и pacemaker. Например, команда (на виртуалке) `ifdown eth0` приводит к зависанию corosync (пишут, что corsync v3 в этом случае не виснет, но не факт). Неисправности сети можно тестировать путем изменения виртуальной среды.
