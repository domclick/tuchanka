...
Поскольку все виртуалки объеденены в группы, согласно кластерам, запускать и останавливать в приложении VirtualBox можно через правую кнопку мыши на имени группы(кластера). Процессы VirtualBox иногда зависают от _Close/ACPI Shutdown_ (помогает `killall -9 VBoxHeadless`, на хосте) и иногда зависает приложение UI (помогает _ESC-Option-Command_). Запускать либо целиком группу _Tuchanka_, либо запуская _Witness4Tuchanka_ и нужную подгруппу, например _Tuchanka0_. Останавливать группу рекомендую через _Close/Power Off_. После этого зайти на любую машину нужного кластера и выполнить команду:
...
Отключение. Сначала можно (но не нужно при _Power Off_) на одной из машин кластера выполнить:

	pcs cluster stop --all

Потом на заголовке группы _Close/Power Off_, так как **от _Close/ACPI Shutdown_ (на группе) VBoxManager иногда виснет**.
